...
from tensorflow.initializers import constant
from tensorflow.keras.models import Model
import tensorflow.keras.backend as K

class AdaptiveWindowingLayer(layers.Layer):
    # def __init__(self, *args, **kwargs):
    #     super().__init__(*args, **kwargs)

    def build(self, input_shape):
        # self.window_level = self.add_weight(
        #     'window_level',
        #     shape=[1],
        #     dtype=tf.float32,
        ...
        #     trainable=True
        # )
        # self.window_width = self.add_weight(
        #     'window_width',
        #     shape=[1],
        #     dtype=tf.float32,
        #     initializer='he_normal',
    ...
    def call(self, input_tensor):
        normalized_tensor = tf.divide(tf.subtract(input_tensor, self.lower), tf.subtract(self.upper, self.lower))
        # normalized_tensor = tf.clip_by_value(normalized_tensor, 0, 1)
        return normalized_tensor

class OctConv3D(layers.Layer):
    def __init__(self, filters, alpha, kernel_size=(3,3,3), strides=(1,1,1), 
    ...
    def call(self, inputs):
        ...
        # High -> High conv
        high_to_high = K.conv3d(high_input, self.high_to_high_kernel,
                                strides=self.strides, padding=self.padding,
                                data_format="channels_last")
        # High -> Low conv
        high_to_low  = K.pool3d(high_input, (2,2,2), strides=(2,2,2), pool_mode="avg")
        high_to_low  = K.conv3d(high_to_low, self.high_to_low_kernel,
                                strides=self.strides, padding=self.padding,
                                data_format="channels_last")
        # Low -> High conv
        low_to_high  = K.conv3d(low_input, self.low_to_high_kernel,
                                strides=self.strides, padding=self.padding,
                                data_format="channels_last")
        ...
        low_to_high = K.repeat_elements(low_to_high, 2, axis=3)
        # Low -> Low conv
        low_to_low   = K.conv3d(low_input, self.low_to_low_kernel,
                                strides=self.strides, padding=self.padding,
                                data_format="channels_last")
        # Cross Add
