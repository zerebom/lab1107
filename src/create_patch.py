...
def main(args):
    ...
    for i, image in enumerate(image_list):
       - EXPERIMENTS: /  # add channel
       - EXPERIMENTS: /  image_array[..., i] = sitk.GetArrayFromImage(image)
 - EXPERIMENTS: /
    la - EXPERIMENTS: /bel_array = np.zeros(tmp_array.shape, dtype=np.int16)
    fo - EXPERIMENTS: /r i, label in enumerate(label_list):
       - EXPERIMENTS: /  if label is not None:
       - EXPERIMENTS: /      # add label by bianry system for overlap more double label
    ...
    re - EXPERIMENTS: /ad_range = image.GetSize()
