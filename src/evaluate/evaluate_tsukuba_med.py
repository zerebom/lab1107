...
def CalcMetrics(ref_array, res_array):
    ...
    dice = 2 * np.count_nonzero(share_array) / (np.count_nonzero(ref_array) + np.count_nonzero(res_array)) if np.count_nonzero(ref_array) + np.count_nonzero(res_array) != 0 else 0.0
...
def main(args):
    ...
    with open(args.output_file, 'w') as f:
        ...
        for reference_file in tqdm(reference_file_list):
            ...
            reference_file_name, dice_per_case, recall_per_case, precision_per_case = EvaluateMain(f, reference_file, resource_file)
